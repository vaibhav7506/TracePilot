      ...
      <div className="container relative grid items-center gap-12 py-16 md:py-24 lg:grid-cols-[1.05fr_1fr] lg:gap-16">
        <Reveal stagger className="flex flex-col items-start">
          ...
          <p className="mt-6 max-w-xl text-base leading-relaxed text-muted-foreground md:text-lg">
            TracePilot QA is an AI browser agent that finds broken user flows before your users do.
            Point it at a URL — it explores like a real person, reproduces the failure, and hands
            you the Playwright test that proves it.
          </p>
          ...
          <ul className="mt-10 flex flex-wrap items-center gap-x-6 gap-y-2 border-t border-border pt-6">
            {capabilities.map((cap) => (
              <li key={cap} className="flex items-center gap-2 font-mono text-xs text-muted-foreground">
        ...
        <Reveal delay={0.12} className="relative">
          <Card className="relative overflow-hidden bg-card/60">
            <div className="flex items-center gap-1.5 border-b border-border px-4 py-3">
              ...
              <span className="h-2.5 w-2.5 rounded-full bg-muted-foreground/40" aria-hidden />
              <span className="ml-3 font-mono text-xs text-muted-foreground">agent://exploring</span>
            </div>
